...
const tokenRefresh = ({ roles = User.roles } = {}) => (req, res, next) =>
    passport.authenticate('tokenRefresh', { session: false }, (err, user, info) => {
...
passport.use('tokenRefresh', new JwtStrategy({
    secretOrKey: jwtSecret,
    jwtFromRequest: ExtractJwt.fromExtractors([
        ExtractJwt.fromUrlQueryParameter('access_token'),
        ExtractJwt.fromBodyField('access_token'),
