...
const getAllComments = async (req, res, next) => {
    ...
    let filter = { film_id: req.params.film_id }
...
const filterComments = async ({ query, params }, res, next) => {
    ...
    if (query.date_start || query.date_end) {
        ...
        if (isNaN(dateStart.getTime())) {
            let message = query.date_start ? { error: 'Bad dateStart format! Format must by DD/MM/YYYYY.' } : { error: 'Starting date cannot be empty!' };
